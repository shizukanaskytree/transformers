...
### This is used to load the ckpt file
### initialize the model with the config
model_config = BertConfig(
    vocab_size=vocab_size,
    max_position_embeddings=max_length,
    num_hidden_layers=1,                    # number of hidden layers of the BERT model
    )
...
################################################################################
### This is used to load the ckpt file
### to init a two layer BERT model
model_config_after_grow = BertConfig(
    vocab_size=vocab_size,
    max_position_embeddings=max_length,
    num_hidden_layers=2,                    # number of hidden layers of the BERT model
    )
model_after_grow = BertForMaskedLM(config=model_config)
################################################################################
...
training_args = TrainingArguments(
    ...
    num_train_epochs=1,            # number of training epochs, feel free to tweak, original code settting is 10
    ...
    save_steps=2,                   # original 1000, for debug and testing 1
...
trainer = Trainer(
    model=model,
    # original_model=model, # new param
    args=training_args,
...
# 怎么变化? 通用, 代码通用, 方便, 简洁?
# 在什么地方插入代码? 变化?
# 在 trainer.train 里面, 因为里面会开始 load ckpt
# 没有的信息就给他补进去

### train the model
trainer.train(resume_from_checkpoint=os.path.join(model_path, "checkpoint-8"))
# trainer.train() # no ckpt case
...
################################################################################
# pipeline usage but not IMPT now.
################################################################################

# ### when you load from pretrained
# model = BertForMaskedLM.from_pretrained(os.path.join(model_path, "checkpoint-2"))
# tokenizer = BertTokenizerFast.from_pretrained(model_path)
